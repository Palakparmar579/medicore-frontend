  ...
  const handleLogin = async (e) => {
    ...
    // Regex method
     ...
     if(!emailPattern.test(form.email)) {
      return seterrorEmail("Please enter a valid email address (example: user@example.com)")
     }
    ...
    if(!passwordPattern.test(form.password)){
      return seterrorPassword("Password must be at least 8 characters long and include uppercase, lowercase, number, and special character.")
    }
    setError("");
    try {
      ...
      <div className="w-sm max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl bg-gradient-to-br from-white/90 to-transparent backdrop-blur-xl shadow-xl rounded-2xl p-6 sm:p-8 md:p-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center text-xs sm:text-sm mb-4 sm:mb-6 gap-2 sm:gap-0">
          <div className="flex items-center">
            ...
            />
            <label htmlFor="remember" className="text-white cursor-pointer text-xs sm:text-sm">
              Remember me
            </label>
          </div>
