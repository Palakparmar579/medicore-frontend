...
const fetchStats = async () => {
    try {
      const response = await axios.get(
        `${backendUrl}/api/department/dashBoardStats`
      );
 ...
 // Pagination
  ...
  try {
    const response = await axios.get(
      `${backendUrl}/api/department/pagination?page=${pageNumber}&limit=${limit}&search=${search}`,
    );
    ...
    try {
      ...
      if (editId) {
        response = await axios.put(
          `${backendUrl}/api/department/editDep/${editId}`,
          {
        ...
        response = await axios.post(
          `${backendUrl}/api/department/registerDep`,
          {
            department: form.department,
            description: form.description,
          },
  ...
  const handleStatusConfirm = async () => {
    try {
      ...
      const response = await axios.put(
        `${backendUrl}/api/department/toggleStatus/${statusId}`,
        {},
        config,
      );
...
<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 sm:mt-10 pt-2 sm:pt-4 gap-3 sm:gap-0">
         ...
         <div className="relative w-full sm:w-auto ">
             ...
             <button
          onClick={handleAdd}
          className="flex items-center gap-2 bg-[#00536e] text-white px-5 py-2 rounded-md hover:bg-[#00455c] hover:scale-90 hover:shadow-xl transition-all duration-300 ease-in-out active:scale-90 active:shadow-md cursor-pointer w-fit"
        >
          <FaUserPlus />
          Add Department
        </button>
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 my-6 sm:my-10">          {cards.map((card, index) => (
            <DashboardCard
              key={index}
              title={card.title}
              count={card.count}
...
<div className="w-100 max-w-[95%] sm:max-w-lg bg-white rounded-2xl shadow-lg overflow-hidden relative">         
            <div className="flex justify-between items-center px-5 sm:px-6 pt-5 pb-3 border-b bg-gradient-to-r from-[#00304e] to-[#005f73] rounded-t-2xl">
...
<div className="overflow-y-auto flex-1 px-4 sm:px-6 py-5 bg-gray-50 max-h-[70vh] sm:max-h-[80vh]">              <form onSubmit={handleSubmit} id="doctor-form">
                <div className="flex flex-col gap-5">
                 
                  {/* DEPARTMENT */}
                  <div className="flex  flex-col sm:flex-row gap-1 sm:gap-4">
                    <label className="text-sm font-semibold sm:w-28 text-gray-700">
            ...
            <div className="flex justify-between items-center px-4 sm:px-6 py-4 border-t bg-white rounded-b-2xl shadow-inner">
              <button
                type="button"
                onClick={() => {
                ...
                className=" cursor-pointer bg-red-500 text-white px-4 sm:px-6 py-2 rounded-lg hover:bg-red-600 hover:scale-105 transition duration-300 shadow-md text-sm"
              >
                Cancel
              </button>
              ...
              <button
                form="doctor-form"
                type="submit"
                className="cursor-pointer bg-gradient-to-r from-[#00304e] to-[#005f73] text-white px-4 sm:px-6 py-2 rounded-lg hover:scale-105 transition duration-300 shadow-md text-sm"
              >
     ...
     <h3 className="text-xl font-semibold bg-gradient-to-r from-[#00304e] to-[#005f73] bg-clip-text text-transparent mb-3">
  Added Departments
</h3>
<div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-4 sm:mt-6">        
        {doctorDetail.length === 0 ? (
          ...
          doctorDetail.map((item) => (
            <div
              key={item._id}
              className="relative group bg-white border border-gray-200 rounded-2xl shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 overflow-hidden"
            >
        ...
        item.status === "active"
          ...
          : "bg-red-100 text-red-600 hover:bg-red-600 hover:text-white"
                  ...
                  <button
                    ...
                    className="  cursor-pointer p-2 rounded-lg bg-gray-100 text-[#00304e] hover:bg-[#00304e] hover:text-white transition-all duration-300 shadow hover:scale-110"
                  >
                    <FaEdit size={13} />
                  </button>
 </div>
